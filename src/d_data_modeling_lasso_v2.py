...
df_model_test["label"] = 1 
reg = LinearRegression(fit_intercept=False).fit(df_model_test.drop(["label"],axis=1), df_model_test["label"])

df_coeff = pd.DataFrame(df_model_test.drop(["label"],axis=1).columns)
...
df_model_test["label"] = 1 
reg = Lasso(fit_intercept=False,alpha=0.0005, positive=True).fit(df_model_test.drop(["label"],axis=1), df_model_test["label"])
